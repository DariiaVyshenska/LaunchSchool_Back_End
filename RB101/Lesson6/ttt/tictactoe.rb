...
def display_game_start
  ...
  msg_size = MESSAGES['major_separator_line'].size
  display_centered_msg(MESSAGES['welcome'], msg_size)
  display_centered_msg(MESSAGES['rounds_req_to_win'], msg_size)
  display_centered_msg(ROUND_WINS.to_s, msg_size)
  display_centered_msg(MESSAGES['minor_separator_line'], msg_size)
  display_centered_msg(MESSAGES['press_enter'], msg_size)
  gets
end
...
def display_game_results(score, winner)
  ...
  msg_size = MESSAGES['major_separator_line'].size
  display_centered_msg(MESSAGES['score'], msg_size)
  display_centered_msg(MESSAGES['player_vs_comp'], msg_size)
  display_centered_msg("#{score['player']}:#{score['computer']}", msg_size)
  display_centered_msg(MESSAGES['minor_separator_line'], msg_size)
  display_centered_msg("#{winner.upcase} #{MESSAGES['won']}", msg_size)
end
